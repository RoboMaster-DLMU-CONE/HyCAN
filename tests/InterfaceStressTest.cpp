...
constexpr unsigned int SEND_INTERVAL_MICROSECONDS =
    1; // Delay between sends per thread (0 for max effort)
...
// --- Callback Function ---
// This callback is invoked by the Reaper thread when a message for the
// registered CAN ID is received.
void stress_test_callback(can_frame frame, const int interface_index,
                          canid_t expected_can_id) {
    // frame.can_id should match expected_can_id due to Reaper's dispatching
    if (interface_index >= 0 && static_cast<size_t>(interface_index) <
                                    g_received_message_counts.size()) {
        g_received_message_counts[interface_index].fetch_add(
    ...
    if (s_callback_invocations.fetch_add(1, std::memory_order_relaxed) % 10000
    == 0) { // Log every 10000th invocation std::cout << "Callback on iface " <<
    interface_index
                  << " (ID 0x" << std::hex << expected_can_id << std::dec
                  << "): Total calls ~" << s_callback_invocations.load() <<
    std::endl;
    }
    */
}
...
void sender_worker_fn(const int thread_id, const canid_t base_can_id) {
    ...
    while (!g_stop_sending_flag.load(std::memory_order_acquire)) {
        ...
        uint64_t send_timestamp_ns_count =
            std::chrono::duration_cast<std::chrono::nanoseconds>(
                ...
                .count();
        std::memcpy(frame_to_send.data, &send_timestamp_ns_count,
                    sizeof(send_timestamp_ns_count));
#else
        ...
        if (senders[interface_rr_index]) {
            (void)senders[interface_rr_index]
                ->send(frame_to_send)
        ...
        if constexpr (SEND_INTERVAL_MICROSECONDS > 0) {
            std::this_thread::sleep_for(
                std::chrono::microseconds(SEND_INTERVAL_MICROSECONDS));
        }
    }
    g_total_sent_attempts.fetch_add(messages_sent_by_this_thread,
                                    std::memory_order_relaxed);
    std::cout << "Sender thread " << thread_id
              << " finished. Attempted to send " << messages_sent_by_this_thread
              << " messages." << std::endl;
}

int main() {
    std::cout << "--- HyCAN Interface Stress Test ---" << std::endl;
    std::cout << "INFO: This test requires CAP_NET_ADMIN or root privileges."
              << std::endl;
    std::cout << "INFO: Ensure 'vcan' module is loaded (sudo modprobe vcan)."
              << std::endl;
    std::cout << "Config: " << NUM_INTERFACES << " interfaces, "
              << NUM_SENDER_THREADS << " sender threads, "
              << TEST_DURATION_SECONDS << "s duration." << std::endl;
    std::cout << "Messages sent with interval: " << SEND_INTERVAL_MICROSECONDS
              << " us." << std::endl;
    ...
    std::cout << "\n--- Phase 1: Initializing " << NUM_INTERFACES
              << " Interfaces ---" << std::endl;
    for (int i = 0; i < NUM_INTERFACES; ++i) {
        ...
        if (target_can_id >= 2048) {
            ...
            std::cerr << "ERROR: CAN ID 0x" << std::hex << target_can_id
                      << std::dec
                      << " exceeds Reaper's max callback ID limit. Adjust "
                         "BASE_CAN_ID or NUM_INTERFACES."
                      << std::endl;
            return EXIT_FAILURE;
        }

        std::cout << "Creating interface: " << if_name
                  << " listening to CAN ID 0x" << std::hex << target_can_id
                  << std::dec << std::endl;
        try {
            auto interface_ptr =
                std::make_unique<HyCAN::VCANInterface>(if_name);

            // Register callback BEFORE calling up()
            auto callback_for_interface = [i, target_can_id](can_frame frame) {
                stress_test_callback(frame, i, target_can_id);
            };

            (void)interface_ptr
                ->register_callback<can_frame>({target_can_id},
                                               callback_for_interface)
                .or_else([&](const auto &e) {
                    std::cerr << e.message << std::endl;
                });

            std::cout << "Callback registered for " << if_name << " on ID 0x"
                      << std::hex << target_can_id << std::dec << std::endl;
            hycan_interfaces.push_back(std::move(interface_ptr));
        } catch (const std::exception &e) {
            std::cerr << "ERROR: Exception during Interface creation for "
                      << if_name << ": " << e.what() << std::endl;
            ...
            return EXIT_FAILURE; // If any interface fails to init, stop the
                                 // test.
        }
    }
    ...
    for (int i = 0; i < NUM_INTERFACES; ++i) {
        if (hycan_interfaces[i]) {
            ...
            try {
                ...
                std::cerr << "ERROR: Exception during Interface::up() for "
                          << if_name << ": " << e.what() << std::endl;
    ...
    std::this_thread::sleep_for(
        std::chrono::seconds(2)); // Give time for reaper threads to start, etc.

    // 2. Start Sender Threads
    std::cout << "\n--- Phase 2: Starting " << NUM_SENDER_THREADS
              << " Sender Threads ---" << std::endl;
    ...
    std::cout << "\nStress test running for " << TEST_DURATION_SECONDS
              << " seconds..." << std::endl;
    ...
    for (int i = 0; i < NUM_INTERFACES; ++i) {
        if (hycan_interfaces[i]) {
            ...
            try {
                ...
                std::cerr << "ERROR: Exception during Interface::down() for "
                          << if_name << ": " << e.what() << std::endl;
            }
        }
    }
    ...
    std::cout << "Total messages send attempts by all threads: "
              << g_total_sent_attempts.load() << std::endl;
    ...
    for (int i = 0; i < NUM_INTERFACES; ++i) {
        ...
        std::cout << "Interface " << if_name << " (ID 0x" << std::hex
                  << target_can_id << std::dec << "): Received " << count
                  << " messages." << std::endl;
        total_received_across_all_interfaces += count;
    }
    std::cout << "Total messages received across all interfaces: "
              << total_received_across_all_interfaces << std::endl;
#ifdef HYCAN_LATENCY_TEST
    ...
    for (int i = 0; i < NUM_INTERFACES; ++i) {
        if (hycan_interfaces[i]) {
            ...
            if (const HyCAN::Dispatcher::LatencyStats stats =
                    hycan_interfaces[i]->get_reaper_latency_stats();
                ...
                overall_weighted_latency_sum_us +=
                    stats.average_latency_us *
                    static_cast<double>(stats.message_count);
            } else {
                std::cout << "Interface " << if_name
                          << ": No latency messages processed." << std::endl;
            }
        }
    }
    if (overall_latency_messages > 0) {
        const double final_overall_avg_latency_us =
            overall_weighted_latency_sum_us /
            static_cast<double>(overall_latency_messages);
        std::cout << "Overall Average Latency: " << std::fixed
                  << std::setprecision(2) << final_overall_avg_latency_us
                  << " us "
                  << "(" << overall_latency_messages << " total msgs)"
                  << std::endl;
    } else {
        std::cout << "No latency messages processed overall for any interface."
                  << std::endl;
    }
#endif
