...
int main()
{
    ...
    std::cout << "Make sure 'can0' exists and is properly configured, or that you have a CAN device connected." <<
        std::endl;
    std::cout << "The program will attempt to bring up 'can0' with a bitrate of 1000000." << std::endl;
    ...
    // The callback will receive all frames since we provide an empty set of IDs.
    auto result = can_interface.tryRegisterCallback({0x123}, callback)
                               .and_then([&]
                                {
    ...
    if (!result)
    {
        std::cerr << "Failed to set up CAN interface: " << result.error().message << std::endl;
        std::cerr << "Please ensure the 'hycan-daemon' is running and the 'can0' interface exists." << std::endl;
        return 1;
    }

    std::cout << "'can0' is up. Now sending a test frame every second..." << std::endl;
    ...
    while (keep_running)
    {
        ...
        {
            std::cerr << "Failed to send frame: " << send_result.error().message << std::endl;
        }
    ...
    if (!down_result)
    {
        std::cerr << "Failed to bring down CAN interface: " << down_result.error().message << std::endl;
    }
